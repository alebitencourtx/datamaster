...
DIR="/home/jovyan/work/lake/twitter/santander_tts/dt*"
if compgen -G "${DIR}" > /dev/null; then
    partition_path=$(ls -td /home/jovyan/work/lake/twitter/santander_tts/dt*/ | head -1 | head -1)
    part=${partition_path:45:-1} 
    echo $part
else
    part='False'
    echo $part
...
spark-submit --conf spark.dynamicAllocation.enabled=false \
            ...
            /app/twitter/silver/batch/batch_silver_tt_tweets.py $part; exit_code=$?
